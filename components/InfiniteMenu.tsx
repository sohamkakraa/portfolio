...
class InfiniteGridMenu {
  ...
  private initTexture(): void {
    ...
    // Ensure a valid texture is bound immediately, before async images finish loading.
    gl.texImage2D(
      gl.TEXTURE_2D,
      0,
      gl.RGBA,
...
const defaultItems: MenuItem[] = [
  {
    ...
    link: "#photography",
    title: "Gallery",
    description: "Add your photography cover images.",
  },
];
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0, onOpenItem }) => {
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      // Avoid creating WebGL context before layout gives the canvas a real size.
  ...
  const handleButtonClick = () => {
    const itemToOpen = activeItem ?? resolvedItems[0];
    ...
    if (!itemToOpen.link) return;
    if (itemToOpen.link.startsWith("http")) {
      window.open(itemToOpen.link, "_blank");
      return;
    }

    if (itemToOpen.link.startsWith("#")) {
      const target = document.querySelector(itemToOpen.link);
      if (target) {
      ...
      <div className="grid h-full w-full place-items-center p-8">
        <div className="w-full max-w-3xl text-center">
          ...
          <h2 className="mt-3 text-2xl font-semibold text-[color:var(--foreground)]">
            Interactive gallery isn’t supported in this browser.
          </h2>
